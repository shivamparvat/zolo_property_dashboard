...
const ActionScreen: React.FC<ActionModalType> = (props) => {
    ...
    const [coordinates, setCoordinates] = useState<Coordinates>({lat: (data?.coordinates || [])[0] || 22, lng: (data?.coordinates || [])[1] || 78})
        ...
        try {
            if (id) {
                ...
                const getOneProperty = async () => {
                    const res = await ApiFeature.get("ads", id, true);
                    if (res?.status == 200) {
                        if (res?.data)
    ...
    const [formInitData, setFormInitData] = useState<any>({
        ...
        expiry_date: data.expiry_date ? (new Date(data.expiry_date)?.toISOString()?.split('T') || [])[0]:new Date()?.toISOString()?.split('T')[0],
        coordinates: {lat: (data?.coordinates || [])[0] || 22, lng: (data?.coordinates || [])[1] || 78}

    });

    const [selectedFile, setSelectedFile] = useState(data.galleryUrls
    );
    ...
    const onsubmit = async (value: any) => {
        ...
        try {
            ...
            res = await ApiFeature.put('ads', formData, id, true);
